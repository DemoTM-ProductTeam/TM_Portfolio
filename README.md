# 📊 DemoTM-ProductTeam – Data Analytics Projects

Repository này tổng hợp **3 dự án phân tích dữ liệu & nghiên cứu thị trường**, được thực hiện bởi **DemoTM-ProductTeam**, tập trung vào **thị trường sữa Việt Nam** và **đánh giá hiệu quả hoạt động marketing** dựa trên dữ liệu thực tế.

Các dự án được xây dựng theo tư duy **data-driven decision making**, hướng đến việc trả lời các câu hỏi kinh doanh thay vì chỉ dừng lại ở mô tả dữ liệu.

---

## Project Overview

| Project | Chủ đề | Trọng tâm |
|-------|-------|----------|
| **Project01** | Thị trường sữa Việt Nam | Hiệu quả & xu hướng thị trường |
| **Project02** | Thị trường sữa Việt Nam | Phân tích chuyên sâu & đề xuất chiến lược |
| **Project03** | Marketing Analytics | Đánh giá hiệu quả chiến dịch marketing TM |

---

## Project01 – Vietnam Milk Market Performance Analysis

🔗 Repository: https://github.com/DemoTM-ProductTeam/Project01  

### Mục tiêu
Phân tích tổng quan **hiệu quả hoạt động của thị trường sữa Việt Nam**, từ đó xác định các xu hướng chính và bức tranh cạnh tranh toàn ngành.

### Nội dung phân tích
- Tổng quan quy mô và mức độ tăng trưởng của thị trường
- So sánh hiệu suất giữa các phân khúc / nhóm sản phẩm
- Phát hiện các xu hướng tiêu dùng nổi bật
- Trực quan hóa dữ liệu phục vụ decision-making

### Giá trị mang lại
- Cung cấp **high-level market insights**
- Hỗ trợ định hướng chiến lược cho doanh nghiệp trong ngành FMCG
- Làm nền tảng cho các phân tích chuyên sâu ở Project02

---

## Project02 – In-depth Analysis of Vietnam Milk Market

🔗 Repository: https://github.com/DemoTM-ProductTeam/Project02  

### Mục tiêu
Đi sâu hơn vào **hành vi tiêu dùng và cấu trúc thị trường sữa Việt Nam**, nhằm đề xuất các định hướng marketing và kinh doanh phù hợp.

### Nội dung phân tích
- Phân tích thị trường theo vùng miền
- Đánh giá tác động của các yếu tố kinh tế & xã hội
- So sánh mức độ cạnh tranh giữa các thương hiệu
- Xây dựng các insight mang tính chiến lược

### Giá trị mang lại
- Chuyển dữ liệu thành **actionable insights**
- Hỗ trợ xây dựng chiến lược marketing & phân phối
- Minh họa cách data analysis được sử dụng trong market research

---

## Project03 – Marketing Campaign Performance Analysis (TM)

🔗 Repository: https://github.com/DemoTM-ProductTeam/Project03  

### Mục tiêu
Đánh giá **hiệu quả các chiến dịch marketing của doanh nghiệp TM trong 3 tháng gần nhất**, nhằm xác định điểm mạnh, điểm yếu và cơ hội tối ưu.

### Nội dung phân tích
- Phân tích hiệu suất theo từng chiến dịch
- So sánh hiệu quả giữa các kênh marketing
- Đánh giá hiệu quả sử dụng ngân sách
- Trực quan hóa kết quả bằng biểu đồ & báo cáo

### Giá trị mang lại
- Giúp doanh nghiệp hiểu rõ **ROI & performance**
- Hỗ trợ tối ưu ngân sách marketing
- Minh họa tư duy Marketing Analytics trong thực tế

---

## Tools & Skills Demonstrated

- Data Analysis & Business Analytics  
- Market Research & Marketing Analytics  
- Data Visualization & Insight Storytelling  
- Python, pandas, matplotlib / seaborn  
- Structured thinking & problem-solving
